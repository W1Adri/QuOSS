...
| 18 | **El wheel no lleva `data/`, y `data/` se resuelve relativo al checkout.** `[project.scripts]` promete un `quoss` instalable desde la etapa 0; una instalación no encuentra ni las estaciones ni los snapshots | **Vuelta a medir el 2026-09-19, después de escribir la CLI, porque la medida anterior ya no describía el árbol.** `uv build` produce ahora `quoss-0.1.0-py3-none-any.whl` de **648 KB y 90 entradas** (85 antes: seis `quoss/cli/*.py` nuevos menos el `.gitkeep` que ya no hace falta), y sigue sin llevar `data/ogs.yaml` ni `data/snapshots/`. **Lo que ha cambiado es que ahora sí se llega a los datos:** la medida de §42 decía «el comando instalado revienta antes de llegar», porque `quoss.cli.main` no existía; instalado el wheel en un `.venv` limpio y fuera del checkout, `quoss run` corre los tres escenarios probados —horizontal, bajada y TLE— y sale 0. Lo que falla es lo que toca `data/`: `load_station_catalogue()` resuelve `DEFAULT_CATALOGUE_PATH` a `<venv>/lib/python3.13/data/ogs.yaml` —un nivel por encima de `site-packages`— y levanta `DataError: Station catalogue not found at …`. No es silencioso, y es una promesa incumplida. **Sigue abierta** porque cerrarla es declarar datos de paquete y escribir el test que lo compruebe **desde una instalación y no desde el árbol**, que es empaquetado. Escrita también en el [ADR 0027](../docs/adr/0027-four-levels-of-distribution.md), «Lo que esto mide» |
